...
static uint8_t temp_args[6][ARG_LENGTH];
...
static int process_pid[6] = {0, 0, 0, 0, 0, 0};
...
void scheduler()
{
    ...
    old_scheduled_coords = terminal_get_location();

    screen_coord_storage[pit_count] = old_scheduled_coords;

    // update pit count
    pit_count++;
    pit_count = (pit_count % 3);

    new_scheduled_coords = screen_coord_storage[pit_count];

    terminal_update_location(new_scheduled_coords.screen_x, new_scheduled_coords.screen_y);
...
//        terminal_update_display_location(new_scheduled_coords.screen_x, new_scheduled_coords.screen_y);
//    }
    ...
    if (current_pid == terminal_process[terminal_active_id])
    {
        ...
        page_table_2_vidmap[0].page_base_addr = PHYSICAL_VID_MEM_PAGE;
    }
    else
    {
        page_table[PHYSICAL_VID_MEM_PAGE].present = 1;
        ...
        page_table_2_vidmap[0].page_base_addr = PHYSICAL_VID_MEM_PAGE + (pit_count + 1);
    }
    flush_TLB();
...
/*    // Clear all FDs in the currrent process
    ...
    if (pcb->parent_pid == -1) 
    {
        terminal_process[terminal_active_id] = -1;
        process_pid[current_pid] = 0;
        execute((uint8_t *)"shell");
        return 1;
    }
    else
    {
        ...
        terminal_process[terminal_active_id] = current_pid;
        ...
        map_paging(current_pid); // restore paging for the parent process
...
<<<<<<< HEAD
    terminal_process[pit_count] = pcb->parent_pid;
    current_pid = pcb->parent_pid;
    process_pid[pcb->pid] = 0;
...
<<<<<<< HEAD
    if ((terminal_process[pit_count] < 0))
    {
...
int32_t execute(const uint8_t *command)
{
    ...
    pcb_t *cur_PCB = (pcb_t *)(MEM_8MB - (current_pid + 1)*MEM_8KB); // 8MB - (i+1) * 8KB E.g. PCB for process 0 starts at 8MB - 8KB
...
>>>>>>> abe9467668c915942dca7b5b2bc0fce4ac573ec9
    ...
    for (i = 0; i < ARG_LENGTH; i++)
    {
        cur_PCB->args[i] = temp_args[current_pid][i];
    }
    // process_total++;
...
<<<<<<< HEAD
    tss.esp0 = MEM_8MB - MEM_8KB * (current_pid) - 0x4; //process kernel mode stack E.g. total = 1, then only have P0, then should be bottom of 8MB
...
int32_t getargs(uint8_t *buf, int32_t nbytes)
{
    ...
    pcb_t *curr_pcb = (pcb_t *)(MEM_8MB - (current_pid + 1)*MEM_8KB);
...
int parse_arguments_of_program(const uint8_t *command, int idx)
{
    ...
    {
        temp_args[current_pid][i] = command[idx] != '\n' ? command[idx] : 0;
    }
...
int32_t vidmap(uint8_t **screen_start)
{
    ...
    uint32_t B8_mapping = page_table[0xB8].page_base_addr;
...
<<<<<<< HEAD
        page_table_2_vidmap[i].page_base_addr = B8_mapping; // points to same physical video
=======
