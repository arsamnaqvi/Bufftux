...
=======
>>>>>>> abe9467668c915942dca7b5b2bc0fce4ac573ec9
...
/*
 * int terminal_update_display_location();
 * Inputs: int x, int y
 * Return Value: 0 on success, -1 on failure
 * Function: force resets cursor, 
 * 			does not force reset screen coords.
 */
int terminal_update_display_location(int x, int y)
{
	...
	terminal_update_cursor(display_x, display_y);
...
<<<<<<< HEAD
/*
 * int terminal__display_scroll_up();
 * Inputs: none
 * Return Value: 0 on success, -1 on failure
 * Function: scroll the terminal up one line
 *			 when its called by putc. putc
 ...
 *			 on screen.
 */
int terminal_display_scroll_up()
{
	...
	{																									// 0 because array starts at index 0, -1 because we dont need to copy
																										// into last line
		*(uint8_t *)(display_video_mem + (i << 1)) = *(uint8_t *)(display_video_mem + ((i + NUM_COLS) << 1));			//1 because need to shift to calculate correct
																										// mem location. and add 1 to offset to attribute location.
		*(uint8_t *)(display_video_mem + (i << 1) + 1) = *(uint8_t *)(display_video_mem + (((i + NUM_COLS) << 1)) + 1); // probably = ATTRIB, but we
	}																									//  copy it over anyway from original
	for (i = (NUM_ROWS - 1) * NUM_COLS; i < NUM_ROWS * NUM_COLS; i++)
	{													 //   location
		*(uint8_t *)(display_video_mem + (i << 1)) = ' ';		 //1 because need to shift to calculate correct, and ' ' because need empty space
		*(uint8_t *)(display_video_mem + (i << 1) + 1) = ATTRIB; // mem location. and add 1 to offset to attribute location.
	}

	terminal_update_display_location(START, LAST_ROW_INDEX); //last row
...
// Assumption: Cursor location same as
// screen coords
screen_coord_info_t terminal_get_display_location()
{
	screen_coord_info_t info;
	info.screen_x = display_x;
	info.screen_y = display_y;
	return info;
}
...
int terminal_switch(uint8_t terminal_id)
{
	...
	void *video_address_main = (void *)(0xB8000);
...
<<<<<<< HEAD
	screen_coord_info_t old_display_screen_coords;
	screen_coord_info_t new_display_screen_coords;
//	screen_coord_info_t old_scheduled_coords;
//	screen_coord_info_t new_scheduled_coords;

	old_display_screen_coords = terminal_get_display_location();

	// store old_screen_coords
	screen_coord_storage[terminal_active_id] = old_display_screen_coords;

    uint32_t temp = page_table[PHYSICAL_VID_MEM_PAGE].page_base_addr;
...
<<<<<<< HEAD
	//map_paging(terminal_process[terminal_active_id]);
...
/*    if (current_pid == terminal_process[terminal_active_id])
    ...
    else
    {
        ...
        page_table[PHYSICAL_VID_MEM_PAGE].read_write = 1;
        page_table[PHYSICAL_VID_MEM_PAGE].user_supervisor = 0;
        page_table[PHYSICAL_VID_MEM_PAGE].page_base_addr = PHYSICAL_VID_MEM_PAGE + (pit_count + 1);
    }
*/
=======
>>>>>>> abe9467668c915942dca7b5b2bc0fce4ac573ec9
...
<<<<<<< HEAD
	new_display_screen_coords = screen_coord_storage[terminal_id]; // we load the coords here,
														   // and we can now safely set the cursor
														   // finalizing everything


	terminal_update_display_location(new_display_screen_coords.screen_x, new_display_screen_coords.screen_y);

	terminal_active_id = terminal_id; // now that we've switched over to this terminal,
									  // set it as the active one
	
	//display_video_mem_index = PHYSICAL_VID_MEM_PAGE + terminal_active_id + 1;
	
	page_table[PHYSICAL_VID_MEM_PAGE].page_base_addr = temp;
	flush_TLB();
...
>>>>>>> abe9467668c915942dca7b5b2bc0fce4ac573ec9
	...
	uint32_t term_0_address_temp = 0xB9000;
	uint32_t term_1_address_temp = 0xBA000;
	uint32_t term_2_address_temp = 0xBB000;
<<<<<<< HEAD
//	uint32_t display_address = 0xB7000;
=======
>>>>>>> abe9467668c915942dca7b5b2bc0fce4ac573ec9
...
<<<<<<< HEAD
//	void *display_address_ready = (void *)display_address;
=======
>>>>>>> abe9467668c915942dca7b5b2bc0fce4ac573ec9
...
<<<<<<< HEAD
/*	page_table[0xB7].present = 1;		  // this page is now present, so 1
	page_table[0xB7].read_write = 1;	  // because its present, we also want to read/write
	page_table[0xB7].user_supervisor = 0; // 0 for only supervisor/kernel access (us)
	page_table[0xB7].page_write_through = 0;
	page_table[0xB7].page_cashed_disabled = 0;
	page_table[0xB7].accessed = 0;
	page_table[0xB7].dirty = 0;
	page_table[0xB7].page_attribute_table = 0;
	page_table[0xB7].global_bit = 0;
	page_table[0xB7].available = 0;
	page_table[0xB7].page_base_addr = 0xB8; // Same mapping between virtual/physical
*/
=======
>>>>>>> abe9467668c915942dca7b5b2bc0fce4ac573ec9
	...
	memset(term_0_address_temp_ready, 0, 4096);
	memset(term_1_address_temp_ready, 0, 4096);
	memset(term_2_address_temp_ready, 0, 4096);
<<<<<<< HEAD
//	memset(display_address_ready, 0, 4096);

	int k;
	for (k = 0; k < 3; k++)
	{
		screen_coord_storage[k].screen_x = 0;
		screen_coord_storage[k].screen_y = 0;
	}
=======
	...
	for (i = 0; i < 3; i++)
	{
		screen_coord_storage[i].screen_x = 0;
		screen_coord_storage[i].screen_y = 0;
	}
